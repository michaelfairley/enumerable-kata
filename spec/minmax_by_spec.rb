...
ruby_version_is "1.8.7" do
  describe "Enumerable#minmax_by" do
    ...
    it "calls each once" do
      enum = EnumerableSpecs::EachCounter.new("3", "11", "4", "2")
      enum.minmax_by(&:to_i).should == ["2", "11"]
      enum.times_called.should == 1
    end

    it "yields once per element" do
      enum = EnumerableSpecs::EachCounter.new("3", "11", "4", "2")
      enum.minmax_by{ |s| s.to_i }.should == ["2", "11"]
      enum.times_yielded.should == 4
    end
  end
end
