shared_examples_for :enumerable_find do |method_name|
  ...
  it "passes each entry in enum to block while block when block is false" do
    visited_elements = []
    @numerous.send(method_name) do |element|
  ...
  it "calls the ifnone proc when there are no elements" do
    ...
    @empty.send(method_name, fail_proc) {|e| true}.should == "yay"
  end
  ...
  ruby_version_is "1.8.7" do
    it "passes through the values yielded by #each_with_index" do
      [:a, :b].each_with_index.send(method_name) { |x, i| ScratchPad << [x, i]; nil }
    ...
    it "passes the ifnone proc to the enumerator" do
      times = 0
      fail_proc = lambda { times += 1; raise if times > 1; "cheeseburgers" }
      @numerous.send(method_name, fail_proc).each {|e| false }.should == "cheeseburgers"
    end
  end

  it "gathers whole arrays as elements when each yields multiple" do
    multi = EnumerableSpecs::YieldsMulti.new
    multi.send(method_name) {|e| e == [1, 2] }.should == [1, 2]
  end
